...
ajplss <- function(link = list("tanhalf", "log", "identity", "tanh")) {
  ...
  ll <- function(y, X, coef, wt, family, offset = NULL, deriv = 0,
    ...
    ## kappa toward 0 (linear), shape psi toward 0 = vM (ridge), asymmetry nu off
    ## its +/-1 wall (boundary).
    pen <- if (.degen_active(family))
      .lss_map_penalty(family, cbind(xi, kappa, psi, nu), family$map_lambda) else NULL
  ...
  structure(list(family = "ajplss", ll = ll, link = paste(link), nlp = 4,
                 ...
                 ## psi and asymmetry nu keep them in the range where the asymmetric
                 ## Jones-Pewsey normalizer stays fast and non-singular, so c = 1
                 ## holds as the default. kappa stays at the default scale (a
